...
void parseCmdLine(int argc, char* argv[], char *in, char *out, int* savepic, BOOL* flagTrace, int* width, int* height, BOOL* flagQuit);
...
void main(int argc, char* argv[])
{
  ...
  parseCmdLine(argc, argv, in, out, &savepic, &flagTrace, &width, &height, &flagQuit);
  ...
  while (quit == FALSE){
    ...
    while ((XEventsQueued(window->display,QueuedAfterReading) > 0)||((i > nstop)&&(quit == FALSE))||(flagWait == TRUE)){
      report.type = 0;
...
void usage(char* name)
{
    fprintf(stderr," Usage: %s [-width n][-height n][-trace][-pbm][-gif][-ppm][-ps][-all][-quit][in-file][out-file]\n",name);
    ...
    fprintf(stderr,"   -gif      : (optional) saves the pictures as GIF\n");
    ...
    fprintf(stderr,"   -quit     : (optional) quit when finished\n");
    fprintf(stderr,"  keys:\n");
